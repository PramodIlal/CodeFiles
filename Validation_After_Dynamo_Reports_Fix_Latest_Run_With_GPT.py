...
def upload_run_output_dir_to_s3(local_run_dir, s3_output_base):
    ...
    Example:
      ...
      s3_output_base = s3://my-bucket/reports/run_validation
    Uploads files to:
      s3://my-bucket/reports/run_validation/run_20260320_123000/<file>
    """
...
def get_run_timestamp():
    """
    Returns run timestamp string used consistently across validation + reconciliation.
    """
    return datetime.now().strftime("%Y%m%d_%H%M%S")


def write_text_to_s3(content, s3_uri):
    """
    Writes plain text content to S3.
    """
    bucket, key = parse_s3_uri(s3_uri)
    s3 = boto3.client("s3")
    s3.put_object(
    ...
    print(f"[OK] Wrote text file to S3: {s3_uri}")


def write_run_pointer_file(s3_output_base, run_folder_name, report_file_name="validation_report.xlsx"):
    """
    Writes latest run pointer files for reconciliation job to read later.

    Creates:
      s3://bucket/reports/run_validation/latest_validation_run.txt
      s3://bucket/reports/run_validation/latest_validation_report_path.txt
    """
    if not is_s3_path(s3_output_base):
        print("[INFO] OUTPUT_BASE_PATH is not S3. Skipping latest run pointer file.")
        return

    latest_run_path = f"{s3_output_base.rstrip('/')}/latest_validation_run.txt"
    latest_report_path = f"{s3_output_base.rstrip('/')}/latest_validation_report_path.txt"

    run_folder_s3 = f"{s3_output_base.rstrip('/')}/{run_folder_name}"
    report_s3 = f"{run_folder_s3}/{report_file_name}"

    write_text_to_s3(run_folder_s3, latest_run_path)
    write_text_to_s3(report_s3, latest_report_path)
...
def create_run_output_dir(base_output_dir=OUTPUT_BASE_PATH, run_timestamp=None):
    """
...
def main():
    ...
    run_timestamp = get_run_timestamp()
    run_output_dir = create_run_output_dir(actual_output_base_path, run_timestamp=run_timestamp)
    ...
    if is_s3_path(OUTPUT_BASE_PATH):
        ...
        # Write latest run pointer files for reconciliation job
        write_run_pointer_file(
            s3_output_base=OUTPUT_BASE_PATH,
            run_folder_name=run_folder_name,
            report_file_name="validation_report.xlsx"
        )
